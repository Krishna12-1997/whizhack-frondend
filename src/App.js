...
function App() {
  ...
  const footerUri = "http://15.207.20.62:1337/api/footer?populate=left_footer";
  const navbarUri =
  ...
  const blogDetailsUri = `http://15.207.20.62:1337/api/blog?populate=blog.latest_blogs.blog_button, all_blog.all_blog.latest_blogs.Single_blog_details, all_blog.all_blog.latest_blogs.blog_button/${blogId}, all_blog.all_blog.latest_blogs.Single_blog_details/${blogId}`;
  const whitePaperUri =
  ...
  const termsAndConditionUri = "http://15.207.20.62:1337/api/terms-condition";
  const ransomwareUri =
