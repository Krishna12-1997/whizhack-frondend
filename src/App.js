...
function App() {
  ...
  const homeUri =
    "http://whizhack.com/api/home?populate=productleft.product_button, whizrangeproduct.button, box.button,contact.partner_contact, contact.product_type, service_compo.service_box.service_homes.service_content, Institution_contact.product_type,Institution_contact.cityname, Institution_contact.partner_contact, career.link ,career.career_right, link, images ";
  const footerUri = "http://whizhack.com/api/footer?populate=left_footer";
  const navbarUri =
    " http://whizhack.com/api/top-right-menu?populate=body.sections.links, body.single_sections";
  const aboutUri =
    "http://whizhack.com/api/about?populate=header.management,header.story,header.Advisor,header.alliance.strategicalliances,header.partnership, header.recognition.recognitions.button";
  const productsUri =
    " http://whizhack.com/api/solution?populate=solution.threat.card, solution.bg_image_url.about_cybers, solution.bg_image_url.card, solution.threat.traces.feature_trace, solution.threat.traces, solution.threat.trace_benefits.trace_benefit , solution.threat.trace_benefits.benefit_card, solution.threat.solution_cases.case_impact, solution.bg_image_url.solution_cases.case_impact, solution.bg_image_url.trace_features.feature_trace, solution.hids_section.about_cybers, solution.hids_section.hids_features.feature_hids";
  const trainingUri =
    "http://whizhack.com/api/training?populate=training.cyber_gurus.price , training.about_cybers.cyber_link, training.cyber_modules.cyber_module, training.btogs.btog_card, training.solution_cases.case_impact";
  const serviceUri =
    "http://whizhack.com/api/service?populate=section, security_zone.service_links.assessment, timeline.timelines, testing, Service_security.service_homes.service_content";
  const blogUri =
    "http://whizhack.com/api/blog?populate=blog.latest_blogs.blog_button, all_blog.all_blog.latest_blogs.blog_button,  all_blog.all_blog.latest_blogs.Single_blog_details";
  const blogDetailsUri = `http://whizhack.com/api/blog?populate=blog.latest_blogs.blog_button, all_blog.all_blog.latest_blogs.Single_blog_details, all_blog.all_blog.latest_blogs.blog_button/${blogId}, all_blog.all_blog.latest_blogs.Single_blog_details/${blogId}`;
  const whitePaperUri =
    "http://whizhack.com/api/white-paper?populate=white_paper_cards";
  const partnersUri =
    "http://whizhack.com/api/partner?populate=partner_contents.partner_card";
  const serviceItUri =
    "http://whizhack.com/api/service-it?populate=service_IT_content";
  const serviceOtUri =
    "http://whizhack.com/api/service-ot?populate=service_ot_content";
  const careerUri =
    "http://whizhack.com/api/career-pathway?populate=career_path_video, career_programs.duration, career_programs.module";
  const termsAndConditionUri = "http://whizhack.com/api/terms-condition";
  const ransomwareUri =
    "http://whizhack.com/api/ransomeware?populate=ransomeware_content.ransomeware_content_inner";
  const investorRelationUri =
    "http://whizhack.com/api/investor-relation?populate=year.investors.investor_shareholder_content, advisorpartner";
  const contactUri = "http://whizhack.com/api/contact-form";
  const industryUri =
    "http://whizhack.com/api/industry?populate=industry_left.industry_lefts, section, industry_right.industry_rights.button";
  const privacyUri = "http://whizhack.com/api/privacy-policy";
